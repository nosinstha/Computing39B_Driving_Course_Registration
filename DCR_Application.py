...
def DCR_admin():
    ...
    def onclick_home_btn():
        ...
        OptionMenu(filter, clicked_course_category, "Booked", "Unbooked").pack(side=LEFT, padx=5)
        ...
        def query():
            ...
            for record in records:
                ...
                c.execute("SELECT COUNT(*) FROM reservation WHERE trainer_id=?", (trainer_id,))
                reserve_count = c.fetchone()[0]
                category = "Booked" if reserve_count >= seats else "Unbooked"
                
                if clicked_course_category.get() != "---Course Category---":
                    if clicked_course_category.get() != category:
                        continue
                ...
                Label(details,
                  text=(f"Vehicle     : {record[4]}\n"
                        ...
                        f"Seats Left  : {seats - reserve_count}\n"
                        f"Status      : {category}\n\n"
        ...
        def open_booking(trainer_id):
            ...
            def save():
                ...
                c.execute("""
                    INSERT INTO reservation (full_name, phone_number, gmail, trainer_id)
                    VALUES (?, ?, ?, ?)
                    """, (
    ...
    def onclick_record_btn():
        ...
        def add():
            ...
            seats = c.fetchone()[0]
            c.execute("SELECT COUNT(*) FROM reservation WHERE trainer_id=?", (TrainerID.get(),))
            reserve_count = c.fetchone()[0]
            ...
            c.execute("INSERT INTO reservation (full_name, phone_number, gmail, trainer_id) VALUES (?, ?, ?, ?)",
        ...
        def query():
            ...
            c.execute(
            """SELECT r.full_name, r.phone_number, r.gmail,
            ...
            headings = [
                "Reserver Name", "Reserver Phone No", "Reserver Gmail",
                "Trainer Name", "Trainer Phone No", "Trainer Gmail",
                "Vehicle", "Duration", "Time Shift", "Reservation ID"
                ]
        ...
        def delete():
            ...
            c.execute("SELECT reservation_id FROM reservation WHERE reservation_id = ?", (ReservationID_entry.get(),))
            ...
            c.execute("DELETE FROM reservation WHERE reservation_id = ?", (ReservationID_entry.get(),))
            conn.commit()
            conn.close()
        ...
        def edit():
            ...
            c.execute("SELECT * FROM reservation WHERE reservation_id = ?", (record_id,))
            records = c.fetchall()
            conn.close()
        ...
        def update():
            ...
            seats = c.fetchone()[0]
            c.execute("SELECT COUNT(*) FROM reservation WHERE trainer_id=?", (trainerid_editor.get(),))
            reserve_count = c.fetchone()[0]
            ...
            c.execute("UPDATE reservation SET full_name=?, gmail=?, phone_number=?, trainer_id=? WHERE reservation_id=?", data)
        ...
        Label(crud, text=".", font=("Inter",1), bg="#0f1b4c").pack(pady=5)
        ...
        Label(content, text="Reservation ID:", bg="#000000", fg="#ffffff").grid(row=0, column=0, padx=10, pady=10)
        ReservationID_entry = Entry(content, width=30, bg="#2c3e50", fg="#ffffff")
        ReservationID_entry.grid(row=0, column=1, padx=10, pady=10)

        Button(content, text="Update", bg="#E900E9", fg="#000000", command=edit).grid(row=1, column=0, padx=10, pady=10)
        Button(content, text="Delete", bg="#E900E9", fg="#000000", command=delete).grid(row=1, column=1, padx=10, pady=10)
    ...
    def onclick_trainer_btn():
        ...
        def delete():
            ...
            reservation = c.fetchone()
            if reservation is None:
                messagebox.showwarning("Invalid Reservation ID", "The entered Trainer ID does not exist!")
                conn.close()
                return
    ...
    icons["profile"] = ImageTk.PhotoImage(Image.open("profile.ico").resize((30, 30)))
    profile_btn = Button(sidebar, image=icons["profile"])
    profile_btn.place(x=50, y=440)
